...
export function Info() {
  ...
  const prod = location.state.productId;
  useEffect(() => {
    if (productId) {
      actions.getProductById(prod);
    ...
    <Card className="mt-5 w-11/12">
      ...
      <CardBody>
        ...
        <Typography color="gray" className="mt-1">
          <span className="font-bold">Nombre/Empresa:</span> {products.name}
        </Typography>
        <Typography color="gray" className="mt-1">
          <span className="font-bold">Categoría:</span> {categories && categories.length > 0 ? categories[0].name : 'No disponible'}
        </Typography>
        <Typography color="gray" className="mt-1">
          <span className="font-bold">Descripción:</span> {products.description}
        </Typography>
        <Typography color="gray" className="mt-1">
          {/* <span className="font-bold">Correo electrónico:</span> {store.productDetails.seller.email} */}
        </Typography>
        <Typography color="gray" className="mt-1">
          <span className="font-bold">Provincia:</span> {products.province}
        </Typography>
      </CardBody>
